...
class SetFilmIzle : MainAPI() {
    override var mainUrl              = "https://setfilmizle.uk"
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val search = app.post(url = "${mainUrl}/wp-admin/admin-ajax.php", headers = mapOf("X-Requested-With" to "XMLHttpRequest"), data = mapOf("action" to "ajax_search", "nonce" to nonce, "search" to query))
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (url.contains("/dizi/")) {
            ...
            return newTvSeriesLoadResponse(title, url, TvType.TvSeries, episodes) { this.posterUrl = poster; this.plot = description; this.year = year; this.tags = tags; this.duration = duration; this.recommendations = recommendations; addActors(actors); addTrailer(trailer) }
        }
        return newMovieLoadResponse(title, url, TvType.Movie, url) { this.posterUrl = poster; this.plot = description; this.year = year; this.tags = tags; this.duration = duration; this.recommendations = recommendations; addActors(actors); addTrailer(trailer) }
    }
    ...
    private fun sendMultipartRequest(nonce: String, postId: String, playerName: String, partKey: String, referer: String): Response {
        ...
        val request = Request.Builder().url("${mainUrl}/wp-admin/admin-ajax.php").post(requestBody).apply { addHeader("Referer", referer); addHeader("X-Requested-With", "XMLHttpRequest") }.build()
        return OkHttpClient().newCall(request).execute()
    }

    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        val document = app.get(data).document
        document.select("nav.player a").map { element -> Triple(element.attr("data-player-name"), element.attr("data-post-id"), element.attr("data-part-key").takeIf { it.isNotEmpty() }) }.forEach { (name, sourceId, partKey) ->
            if (sourceId.contains("event") || sourceId == "") return@forEach
