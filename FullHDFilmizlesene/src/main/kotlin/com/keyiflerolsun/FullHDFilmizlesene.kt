...
class FullHDFilmizlesene : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val url = if (request.data.endsWith("/")) request.data else "${request.data}/"
        val document = app.get(url).document

        val home = document.select("li.film").mapNotNull { it.toSearchResult() }
            .ifEmpty {
                document.select(".film, article.film, .movie-item, .film-item, .film-box").mapNotNull { it.toSearchResult() }
            }
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        val title = this.selectFirst("span.film-title")?.text()?.trim()
            ...
            ?: selectFirst(".film-title, .title, h2, h3, h4")?.text()?.trim()
            ?: return null

        val href = fixUrlNull(selectFirst("a")?.attr("href") ?: attr("href")) ?: return null
    ...
    private fun rtt(s: String): String {
        fun rot13Char(c: Char): Char = when (c) {
    ...
    private fun getVideoLinks(document: Document): List<Map<String, String>> {
        ...
        for (key in keys) {
            ...
            when (t) {
                ...
                is Map<*, *> -> {
                    val links = t.mapValues { (_, value) -> if (value is String) atob(rtt(value)) else "" }
                    linkList.add(links.mapKeys { (key, _) -> key?.toString() ?: "Unknown" })
                }
            }
        }
        return linkList
