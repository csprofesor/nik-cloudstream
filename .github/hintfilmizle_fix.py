...
# Build-time patch: add a Kinescope-only WebView network filter. The filter is
# active only when WebViewResolver was invoked with a kinescopecdn.net /embed URL,
# so other CloudStream plugins using WebViewResolver are unaffected.
