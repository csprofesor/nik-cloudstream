...
class HDFilmCehennemi : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val tvType = if (document.select("nav#seasonsTabs").isEmpty()
    ...
    override suspend fun loadLinks(
        ...
        document.select("nav.nav.card-nav.nav-slider a.nav-link, div.alternative-links button.alternative-link, div.card-video > iframe, iframe[data-src], iframe[src]").forEach { el ->
            val url = el.attr("href").ifBlank { el.attr("data-video_url").ifBlank { el.attr("data-src").ifBlank { el.attr("src") } } }
        ...
        sources.forEach { sourceUrl ->
            try {
                val finalUrl = if (sourceUrl.contains("/video/")) {
                    val apiGet = app.get(sourceUrl, referer = data, headers = mapOf("X-Requested-With" to "fetch")).text
                    val iframeMatch = Regex("""data-src=\\"([^"]+)""").find(apiGet)?.groupValues?.get(1)?.replace("\\", "")
                        ?: Regex("""src=['"]([^'"]+)['"]""").find(apiGet)?.groupValues?.get(1)
                    iframeMatch?.let { fixUrl(it) } ?: sourceUrl
                } else {
                    sourceUrl
                }
