...
class HDFilmCehennemi : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val doc = app.get(url, headers = headers, referer = mainUrl).text
    ...
    override suspend fun loadLinks(
        ...
        Log.d("HDCH", "loadLinks çağrıldı, data » $data")
        ...
        document.select("div.alternative-links").map { element ->
            ...
            element.select("button.alternative-link").map { button ->
                ...
                try {
                    ...
                    Log.d("HDCH", "İframe çözüldü: $iframe")

                    loadExtractor(iframe, data, subtitleCallback) { link ->
                        callback.invoke(link)
                    }
                } catch (e: Exception) {
                    Log.e("HDCH", "Link yükleme hatası (videoID: $videoID): ${e.message}")
                }
            }
        }
        return true
