...
/** HintFilmIzle-only WebViewResolver. The class deliberately lives in this
 * plugin package so the plugin can own shouldInterceptRequest without
 * modifying CloudStream's shared WebViewResolver implementation. */
class WebViewResolver(
    ...
    suspend fun resolveUsingWebView(
        ...
        withContext(Dispatchers.Main) {
            webView = WebView(
                (getContext() as? Context)
                    ?: throw RuntimeException("No base context in HintFilmIzle WebViewResolver")
            ).apply {
            ...
            webView!!.webViewClient = object : WebViewClient() {
                ...
                override fun shouldInterceptRequest(
                    ...
                    // Null is intentional: native Android WebView networking handles
                    // Kinescope requests, preserving its cookies/fetch/session state.
        ...
        android.util.Log.i("HintFilmWebView", "TIMEOUT=${timeout}ms")
        destroy()
