...
class HintFilmIzle : MainAPI() {
    ...
    private fun detailTitle(doc: org.jsoup.nodes.Document, url: String): String? {
        ...
        return sequenceOf(
            exactLink?.text(), doc.selectFirst("main h1")?.text(), doc.selectFirst("article h1")?.text(),
            doc.selectFirst("h1")?.text(), doc.selectFirst(".film-title")?.text(),
            doc.selectFirst(".movie-title")?.text(), doc.selectFirst(".entry-title")?.text()
        ).mapNotNull(::cleanTitle).firstOrNull()
    ...
    private fun results(doc: org.jsoup.nodes.Document, slug: String? = null): List<SearchResponse> = doc.select("a[href*='/film/'],a[href*='/dizi/']").mapNotNull { a ->
        val card = cardFor(a); if (slug != null && !categoryMatches(card, slug)) null else a.toResult(card)
    }.distinctBy { it.url }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        listOf("$mainUrl/film?search=$q", "$mainUrl/film?s=$q", "$mainUrl/?s=$q", "$mainUrl/?search=$q", "$mainUrl/arama?q=$q").forEach { url ->
            val r = runCatching { results(app.get(url, referer = "$mainUrl/", headers = headers()).document) }.getOrDefault(emptyList()); if (r.isNotEmpty()) return r
        }
        return emptyList()
    }
    ...
    private fun genres(doc: org.jsoup.nodes.Document, text: String): List<String> {
        val dom = doc.select("a[href*='/tur/'],.genres a,.genre a,.categories a").map { it.text().trim() }.filter { it.isNotBlank() && it.contains("Film", true) }.distinct()
    ...
    private fun actors(doc: org.jsoup.nodes.Document): List<Actor> {
        val links = doc.select("a[href*='/oyuncular/'],a[href*='/oyuncu/'],a[href*='/actor/'],a[href*='/cast/']").map { cleanTitle(it.text()) }.filterNotNull().filter { it.length < 100 }.distinct()
        ...
        return container?.select("a")?.map { cleanTitle(it.text()) }?.filterNotNull()?.filter { it.length < 100 }?.distinct()?.map(::Actor).orEmpty()
    }
    private fun plot(doc: org.jsoup.nodes.Document, text: String): String? {
        val section = Regex("GENEL BAKIŞ\\s+(.*?)(?=BU FİLM ÖZETİ|HATA BİLDİR|FRAGMAN|ÖNE ÇIKAN OYUNCULAR|YÖNETMEN|ÜLKE\\s)", setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL)).find(text)?.groupValues?.getOrNull(1) ?: return null
    ...
    private suspend fun kinescope(kine: String, parent: String, callback: (ExtractorLink) -> Unit): Boolean = runCatching {
        ...
        resolver.resolveUsingWebView(actualKine,referer=parent,headers=mapOf("Referer" to parent,"Origin" to "https://${actualParsed.host}","User-Agent" to ua)){req->
            val u=req.url.toString();if(m3u.containsMatchIn(u)){stream=u;Log.d("HintFilmIzle","KINESCOPE_MANIFEST="+u);true}else false
        }
